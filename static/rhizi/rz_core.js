...
var initDrawingArea = function () {
	...
	// removed this since: we are separating between querying and loading :
...
function query_from_backend(query){
    if (rz_config.backend_enabled) {
        main_graph.query_from_backend(undefined,query);
        //main_graph.load_from_backend();
...
initDrawingArea();
//init_ws_connection(); # WS INIT REMOVED
...
{
	console.log('update_view__graph: relayout: '+relayout)
...
return {
	...
	reset:function(){
	        console.log('rz_core - resetting :');

	        var canvas = $('#canvas_d3');
	        //canvas.remove();

			main_graph.clear(true);
			edit_graph.clear(true);

	        //initDrawingArea();
			main_graph_view.update_view(true);
	    },
	reload:function(query){
	        console.log('rz_core - reloading :');
    ...
    load_from_json: function(result) {
		console.log('load_from_json: result: '+result)
