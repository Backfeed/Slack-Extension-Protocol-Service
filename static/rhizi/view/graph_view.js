...
function GraphView(spec) {
    ...
    function update_view(relayout) {
        ...
        noderef.insert("image")
            .attr("width", "14")
            .attr("height", "14")
            .attr("xlink:href", "rhizi/img/url-icon.png");
        ...
        noderef1.insert("image")
            ...
            .attr("xlink:href", "rhizi/img/plusExpand.png")
			.on("click", function(d, i) {
				d3.event.stopPropagation();
				//event.preventDefault();
				
				// send extract event to deap :
				window.postMessage({ type: "ExpandNodeClicked", node: d }, "*");
        ...
        noderef2.insert("image")
            ...
            .attr("xlink:href", "rhizi/img/feedback.png")
			.on("click", function(d, i) {
				d3.event.stopPropagation();
				//event.preventDefault();

				// send extract event to deap :
				window.postMessage({ type: "FeedbackNodeClicked", node: d }, "*");
        ...
        node.select('g.node circle')
			...
			.attr("stroke", function(d) {
				if(d.avgFeedback){
                	return 'rgb(128,'+ Math.round( (d.avgFeedback-(-50))*2.4 ) +',128)';
				}
				else{
					return 'grey';
				}
            ...
            .attr("r", function(d) {
				var	r = view_helpers.customSize(d.type) - 2;
            ...
            .on("click", function(d, i) {
                ...
                selection.update([d]);
                if(!temporary) {
					// TBD: send event to DEAP or show other buttons
                    //showNodeInfo(d);
                }
            });
